...
namespace Server
{
    internal sealed class Program
    {
        ...
        static async Task Main(string[] args)
        {
            if (args.Length < 2)
            {
                Console.WriteLine("Usage: Server {AVRO|JSON|RAW} iterations [interval_in_seconds]");
                return;
            }
        ...
        {
            AvroComm.AvroModel.AvroModel.TelemetrySender telemetryCollectionSender = new(mqttSessionClient);

            for (int i = 0; i < iterations; i++)
            {
                Console.WriteLine($"  Sending iteration {i}");
                await telemetryCollectionSender.SendTelemetryAsync(new AvroComm.AvroModel.TelemetryCollection
                {
        ...
        {
            JsonComm.JsonModel.JsonModel.TelemetrySender telemetryCollectionSender = new(mqttSessionClient);

            for (int i = 0; i < iterations; i++)
            {
                Console.WriteLine($"  Sending iteration {i}");
                await telemetryCollectionSender.SendTelemetryAsync(new JsonComm.JsonModel.TelemetryCollection
                {
        ...
        {
            RawComm.RawModel.RawModel.TelemetrySender telemetryCollectionSender = new(mqttSessionClient);

            for (int i = 0; i < iterations; i++)
            {
                Console.WriteLine($"  Sending iteration {i}");
                byte[] payload = Encoding.UTF8.GetBytes($"Sample data {i}");
                await telemetryCollectionSender.SendTelemetryAsync(payload);
