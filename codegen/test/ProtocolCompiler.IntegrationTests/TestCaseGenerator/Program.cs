    ...
    internal class Program
    {
        static int Main(string[] args)
        {
            try
            {
                if (args.Length < 4)
                {
                    Console.WriteLine("Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.TestCaseGenerator annexFile modelFile patternFile testCaseFolder [casesPerTest]");
                    return 1;
                }
            ...
            {
                Console.WriteLine($"Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.TestCaseGenerator failed with exception: {ex.Message}");
                return 1;
            }
