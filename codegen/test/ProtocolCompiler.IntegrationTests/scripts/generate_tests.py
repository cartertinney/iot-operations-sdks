...
def gen_schema(model_path, schemas_dir):
    ...
    schema_gen_exe = join('..', '..', '..', 'src', 'Azure.Iot.Operations.ProtocolCompiler', 'Azure.Iot.Operations.ProtocolCompiler.SchemaGenerator', 'bin', 'Debug', 'net7.0', 'Azure.Iot.Operations.ProtocolCompiler.SchemaGenerator.exe')
...
def gen_from_json(lib_path, schemas_dir, namespace):
    ...
    type_gen_exe = join('..', '..', '..', 'src', 'Azure.Iot.Operations.ProtocolCompiler', 'Azure.Iot.Operations.ProtocolCompiler.TypeGenerator', 'bin', 'Debug', 'net7.0', 'Azure.Iot.Operations.ProtocolCompiler.TypeGenerator.exe')
...
def gen_envoy(lib_path, namespace, annex_path):
    envoy_gen_exe = join('..', '..', '..', 'src', 'Azure.Iot.Operations.ProtocolCompiler', 'Azure.Iot.Operations.ProtocolCompiler.EnvoyGenerator', 'bin', 'Debug', 'net7.0', 'Azure.Iot.Operations.ProtocolCompiler.EnvoyGenerator.exe')
...
def gen_cases(annex_path, model_path, case_dir):
    case_gen_exe = join('..', 'Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.TestCaseGenerator', 'bin', 'Debug', 'net7.0', 'Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.TestCaseGenerator.exe')
...
def gen_test_code(annex_path, model_path, case_dir, test_root):
    code_gen_exe = join('..', 'Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.TestCodeGenerator', 'bin', 'Debug', 'net7.0', 'Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.TestCodeGenerator.exe')
...
run([ 'TextTransform', join(support_dir, 'DotNetTestSln.tt'), '-out', join(gen_root, 'Azure.Iot.Operations.ProtocolCompiler.IntegrationTests.sln') ])
end = time.time()
