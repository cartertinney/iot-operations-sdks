namespace Akri.Dtdl.Codegen
{
    ...
    public class SchemaGenerator
    {
        ...
        public static void GenerateSchemas(IReadOnlyDictionary<Dtmi, DTEntityInfo> modelDict, Dtmi interfaceId, string projectName, DirectoryInfo workingDir, out string annexFile, out List<string> schemaFiles)
        {
            ...
            telemetryTopicCollisionDetector.Check(dtInterface, dtInterface.Telemetries.Keys);
            commandTopicCollisionDetector.Check(dtInterface, dtInterface.Commands.Keys);

            var schemaGenerator = new SchemaGenerator(modelDict, projectName, dtInterface);

            string genNamespace = NameFormatter.DtmiToNamespace(dtInterface.Id);

            List<string> annexFiles = new List<string>();
            schemaGenerator.GenerateInterfaceAnnex(GetWriter(workingDir.FullName, annexFiles));
            annexFile = annexFiles.First();

            schemaFiles = new List<string>();
            schemaGenerator.GenerateTelemetrySchemas(GetWriter(workingDir.FullName, schemaFiles));
            schemaGenerator.GenerateCommandSchemas(GetWriter(workingDir.FullName, schemaFiles));
            schemaGenerator.GenerateObjects(GetWriter(workingDir.FullName, schemaFiles));
            schemaGenerator.GenerateEnums(GetWriter(workingDir.FullName, schemaFiles));
        ...
        public SchemaGenerator(IReadOnlyDictionary<Dtmi, DTEntityInfo> modelDict, string projectName, DTInterfaceInfo dtInterface)
        {
            this.modelDict = modelDict;
            this.projectName = projectName;
            this.dtInterface = dtInterface;

            payloadFormat = (string)dtInterface.SupplementalProperties[DtdlMqttExtensionValues.PayloadFormatProperty];
            genNamespace = NameFormatter.DtmiToNamespace(dtInterface.Id);

            telemetryTopic = dtInterface.SupplementalProperties.TryGetValue(DtdlMqttExtensionValues.TelemTopicProperty, out object? telemTopicObj) ? (string)telemTopicObj : null;
            commandTopic = dtInterface.SupplementalProperties.TryGetValue(DtdlMqttExtensionValues.CmdReqTopicProperty, out object? cmdTopicObj) ? (string)cmdTopicObj : null;
            separateTelemetries = telemetryTopic?.Contains(MqttTopicTokens.TelemetryName) ?? false;
        }

        public void GenerateInterfaceAnnex(Action<string, string, string> acceptor)
        {
            ...
            List<(string, string?, string?, bool, string?)> cmdNameReqRespIdemStales = dtInterface.Commands.Values.Select(c => (c.Name, GetRequestSchema(c), GetResponseSchema(c), IsCommandIdempotent(c), GetTtl(c))).ToList();
        ...
        public void GenerateTelemetrySchemas(Action<string, string, string> acceptor)
        {
            if (dtInterface.Telemetries.Any())
            {
                ...
                {
                    List<(string, string, DTSchemaInfo, int)> nameDescSchemaIndices = dtInterface.Telemetries.Values.Select(t => (t.Name, t.Description.FirstOrDefault(t => t.Key.StartsWith("en")).Value ?? $"The '{t.Name}' Telemetry.", t.Schema, GetFieldIndex(t))).ToList();
        ...
        public void GenerateCommandSchemas(Action<string, string, string> acceptor)
        {
            foreach (KeyValuePair<string, DTCommandInfo> dtCommand in dtInterface.Commands)
            {
                string? reqSchema = null;
                if (dtCommand.Value.Request != null)
                {
                ...
                if (dtCommand.Value.Response != null)
                {
        ...
        public void GenerateObjects(Action<string, string, string> acceptor)
        {
            foreach (DTObjectInfo dtObject in modelDict.Values.Where(e => e.EntityKind == DTEntityKind.Object).Select(e => (DTObjectInfo)e))
            {
                ...
                List<(string, string, DTSchemaInfo, int)> nameDescSchemaIndices = dtObject.Fields.Select(f => (f.Name, f.Description.FirstOrDefault(t => t.Key.StartsWith("en")).Value ?? $"The '{f.Name}' Field.", f.Schema, GetFieldIndex(f))).ToList();
                nameDescSchemaIndices.Sort((x, y) => x.Item4 == 0 && y.Item4 == 0 ? x.Item1.CompareTo(y.Item1) : y.Item4.CompareTo(x.Item4));
                int ix = nameDescSchemaIndices.FirstOrDefault().Item4;
                nameDescSchemaIndices = nameDescSchemaIndices.Select(x => (x.Item1, x.Item2, x.Item3, x.Item4 == 0 ? ++ix : x.Item4)).ToList();
        ...
        public void GenerateEnums(Action<string, string, string> acceptor)
        {
        ...
        {
            return dtEntity.SupplementalTypes.Contains(DtdlMqttExtensionValues.IndexedAdjunctType) ? (int)dtEntity.SupplementalProperties[DtdlMqttExtensionValues.IndexProperty] : 0;
        }
        ...
        private string? GetResponseSchema(DTCommandInfo dtCommand)
        {
        ...
        private static bool IsCommandIdempotent(DTCommandInfo dtCommand)
        {
            return dtCommand.SupplementalTypes.Contains(DtdlMqttExtensionValues.IdempotentAdjunctType);
        }

        private static string? GetTtl(DTCommandInfo dtCommand)
        {
            return dtCommand.SupplementalTypes.Contains(DtdlMqttExtensionValues.CacheableAdjunctType) ? XmlConvert.ToString((TimeSpan)dtCommand.SupplementalProperties[DtdlMqttExtensionValues.TtlProperty]) : null;
        }
