        ...
        where T2 : IMessage<T2>, new()
    {
            ...
            where T : class
        {
            try
            {
                ...
                {
                    return new(new((payload as IMessage<T1>).ToByteArray()), ContentType, PayloadFormatIndicator);
                }
                else if (typeof(T) == typeof(T2))
                {
                    return new(new((payload as IMessage<T2>).ToByteArray()), ContentType, PayloadFormatIndicator);
                }
                else
                {
