...
use crate::{
    ...
    common::{
        ...
        hybrid_logical_clock::{HybridLogicalClock, HLCErrorKind},
        is_invalid_utf8,
        payload_serialize::{
...
fn validate_and_parse_response<TResp: PayloadSerialize>(
        ...
        .find(|(key, _)| UserProperty::from_str(key) == Ok(UserProperty::ProtocolVersion))
    {
        if let Some(response_version) = ProtocolVersion::parse_protocol_version(protocol_version) {
            ...
            log::error!("Received a response with an unparsable protocol version number: {protocol_version}");
            return Err(RPCError::new(
                RPCErrorKind::UnsupportedVersion {
                    protocol_version: protocol_version.to_string(),
                    supported_major_versions: SUPPORTED_PROTOCOL_VERSIONS.to_vec()
                },
                None,
                false,
                command_name,
                ...
                match HybridLogicalClock::from_str(&value) {
                    Ok(ts) => {
                        ...
                        if let Err(e) = application_hlc.update(&ts) {
                            let property_name = match e.kind() {
                                HLCErrorKind::OverflowWarning => "Counter",
                                HLCErrorKind::ClockDrift => "MaxClockDrift"
                            };
                            return Err(RPCError::new(
                                RPCErrorKind::StateInvalid {
                                    property_name: property_name.to_string(),
                                    property_value: None,
                                },
                                Some(e),
                                false, // TODO: this was originally true
                                command_name
                            ))
                        }
                        timestamp = Some(ts);
